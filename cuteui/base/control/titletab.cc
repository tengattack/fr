...
#define TT_DEFAULT_TAB_HEIGHT	30
...
namespace view{
	namespace frame{
		...
		CTitleTab::CTitleTab()
			...
			, m_tab_height(TT_DEFAULT_TAB_HEIGHT)
		{
		}
		CTitleTab::~CTitleTab()
		{
		...
		LRESULT CALLBACK CTitleTab::OnWndProc(UINT message, WPARAM wParam, LPARAM lParam, bool& handled)
		{
			...
			case WM_LBUTTONDOWN:
				if (!m_lbuttondown)
				{
					...
					{
						//x point
						int i = HitTest(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
						if (i != m_downed_)
						{
		...
		bool CTitleTab::CreateTitleTab(HWND hParent, LPWSTR lpszText, unsigned short id, 
			int x, int y, int width, int height, TitleTabStyle style, DWORD dwStyle)
		{
			m_style = style;
		...
		//function
		int CTitleTab::AddTab(LPCWSTR lpszText, int width, int height)
		{
		...
		void CTitleTab::SetTabHeight(int height)
		{
			m_tab_height = height;
		}
		...
		void CTitleTab::UIUpdate(bool force_)
		{
			if (m_need_update || force_)
			{
				...
				for (unsigned long i = 0; i < m_tab.size(); i++)
				{
					...
					if (m_tab[i].width == -1) {
						...
						if (m_style == kStyleHorizontal) {
							...
							*/
							...
							m_tab[i].width = m_width;
						}
					}

					if (m_tab[i].height == -1) {
						if (m_style == kStyleHorizontal) {
							m_tab[i].height = m_height;
						} else {
							m_tab[i].height = m_tab_height;
						}
					}

					if (m_style == kStyleHorizontal) {
						m_tab[i].left = tab_left;
						tab_left += m_tab[i].width;
					} else {
						m_tab[i].top = tab_top;
						tab_top += m_tab[i].height;
					}
					...
					/*RectF layout(text_left, 1, m_tab[i].width, m_tab[i].height - 1);
					...
					Gdip::CreateGlowTextBitmap(&bmptext, global::Gdip::font, global::Gdip::fontfamily, (FontStyle)0, 13 * 1.4f, global::Gdip::strfmt, 
							...
							m_tab[i].width - 1, m_tab[i].height - 1,
					...
					*/
					...
					SkiaDrawLinear(canvas, SKIA_LINEAR_MODE_VERTICAL, 
						(SkColor *)backcolors, positions, 2,
						m_tab[i].left, m_tab[i].top, m_tab[i].left + m_tab[i].width, m_tab[i].top + m_tab[i].height);

					canvas.drawText(m_tab[i].text.c_str(), text_length << 1, 
						m_tab[i].left + ((float)m_tab[i].width - textrect.width()) / 2 - textrect.left(), m_tab[i].top + ((float)m_tab[i].height + textrect.height()) / 2 - textrect.bottom(), font_paint);
		...
		void CTitleTab::DrawSelectTab(SkCanvas& canvas, TA_TITLE_TAB_ITEM *pItem, BYTE alpha, int iIndex)
		{
...
#if 0
			LinearGradientBrush lbbg(Gdiplus::Rect(2, 2, pItem->width-4, pItem->height-4),
					Color::WhiteSmoke, Color::Transparent,
			...
			g->DrawString(pItem->text.c_str(), -1, global::Gdip::font_titletab,
				RectF(0, 1, pItem->width, pItem->height), global::Gdip::strfmt, skin::CSkinData::m_tti.select_text_brush);
...
#endif
			SkiaDrawLinear(canvas, SKIA_LINEAR_MODE_VERTICAL,
				(SkColor *)colors, (SkScalar *)&skin::CSkinData::m_tti.select_lcolor.pos[0], skin::CSkinData::m_tti.select_lcolor.n,
				0, 0, pItem->width, pItem->height);

			delete[] colors;

			SkRect rc = {1, 1, pItem->width + 2, pItem->height + 2};

			SkPaint paint;
			paint.setColor(skin::CSkinData::m_tti.select_text_color.GetValue());
			...
			canvas.drawText(pItem->text.c_str(), text_len << 1, 
				((float)pItem->width - textrect.width()) / 2 - textrect.left(), ((float)pItem->height + textrect.height()) / 2 - textrect.bottom(), paint);
			...
			if (m_style == kStyleHorizontal) {
				canvas.drawLine(0, 1, pItem->width, 1, pen);
			} else if (m_style == kStyleVertical) {
				canvas.drawLine(1, 0, 1, pItem->height, pen);
			}
		}
		...
		void CTitleTab::DrawHoverTab(SkCanvas& canvas, TA_TITLE_TAB_ITEM *pItem, BYTE alpha, int iIndex)
		{
			...
			/*LinearGradientBrush lbback(Rect(0, 0, pItem->width, pItem->height),
							Color::WhiteSmoke, Color::Transparent,
							LinearGradientModeVertical);
			lbback.SetInterpolationColors(backcolors, positions, 2);
			g->FillRectangle(&lbback, 0, 0, pItem->width, pItem->height);*/
		...
		void CTitleTab::DrawTabs(int* pIndexs, int count, HDC hdc)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (pIndexs[i] != m_downed_ && pIndexs[i] != m_hovered_ && pIndexs[i] != m_selected_)
				{
					if (pItem->fade >= 255)
					{
						doSkPaint(_hdc, pItem->left, pItem->top, pItem->width, pItem->height, pItem->left, pItem->top);
						continue;
					}
				}

				SkBitmap tabbitmap;
				tabbitmap.setConfig(SkBitmap::kARGB_8888_Config, pItem->width, pItem->height);
				tabbitmap.allocPixels();

				SkCanvas canvas(tabbitmap);
				SkRect srcrect = {pItem->left, pItem->top, pItem->left + pItem->width, pItem->top + pItem->height};
				SkRect dstrect = {0, 0, pItem->width, pItem->height};
				...
				SkiaPaint(_hdc, pItem->left, pItem->top, pItem->width, pItem->height, tabbitmap, 0, 0);
			}
		...
		int CTitleTab::HitTest(int x, int y)
		{
			for (unsigned long i = 0; i < m_tab.size(); i++)
			{
				if (x >= m_tab[i].left && x <= m_tab[i].left + m_tab[i].width &&
					y >= m_tab[i].top && y <= m_tab[i].top + m_tab[i].height)
				{
					return i;
					break;
				}
