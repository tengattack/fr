...
std::stack<LogAssertHandlerFunction> log_assert_handler_stack;
...
#elif _DEBUG
  g_vlog_info = new VlogInfo("1", "", &g_min_log_level);
#endif
...
ScopedLogAssertHandler::ScopedLogAssertHandler(
  ...
  log_assert_handler_stack.push(std::move(handler));
}

ScopedLogAssertHandler::~ScopedLogAssertHandler() {
  log_assert_handler_stack.pop();
}
...
#if defined(OS_WIN)
  MessageBoxW(nullptr, UTF8ToUTF16(str).c_str(), L"Fatal error",
...
LogMessage::~LogMessage() {
  ...
  if (severity_ == LOG_FATAL && !base::debug::BeingDebugged()) {
    ...
    if (!(stream_.str().find("Backtrace:") != std::string::npos)) {
...
#undef ASL_LEVEL_STR_X
  ...
  if (severity_ == LOG_FATAL) {
    ...
    if (!log_assert_handler_stack.empty()) {
      LogAssertHandlerFunction log_assert_handler =
          log_assert_handler_stack.top();
...
std::ostream& std::operator<<(std::ostream& out, const wchar_t* wstr) {
  return out << (wstr ? WideToUTF8(wstr) : std::string());
}
