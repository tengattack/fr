...
static const JSONReader::Token kInvalidToken(JSONReader::Token::INVALID_TOKEN,
                                             0, 0);
//TACHANGE 100
static const int kStackLimit = 0x2000;
    ...
    case Token::ARRAY_BEGIN:
      {
        ...
        while (token.type != Token::ARRAY_END) {
		  ...
		  //TengAttack Change TACHANGE for like this [,,,,]
		  if (!array_node) {
			Token ttoken = ParseToken();
			if (ttoken.type == Token::LIST_SEPARATOR) {
              array_node = Value::CreateNullValue();
			}
		  }
          if (!array_node)
            return NULL;
    ...
    case Token::OBJECT_BEGIN:
      {
        ...
        while (token.type != Token::OBJECT_END) {
          ...
          if (token.type == Token::STRING) {
            ...
            DCHECK(UTF16ToUTF8(token.begin, token.length, &dict_key));
          } else {
            SetErrorCode(JSON_UNQUOTED_DICTIONARY_KEY, json_pos_);
            return NULL;
          }
