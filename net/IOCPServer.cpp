...
// 
////////////////////////////////////////////////////////////////////////////////
bool CIOCPServer::Initialize(NOTIFYPROC pNotifyProc, LPVOID lParam, int nMaxConnections, int nPort)
{
...
// 	}
	...
	if (setsockopt(pContext->m_Socket, SOL_SOCKET, SO_KEEPALIVE, (char *)&chOpt, sizeof(char)) != 0)
	{
		TRACE(_T("setsockopt() error\n"), WSAGetLastError());
	}
	...
	m_pNotifyProc((LPVOID) m_lParam, pContext, NC_CLIENT_CONNECT);
...
// 
////////////////////////////////////////////////////////////////////////////////
std::string CIOCPServer::GetHostName(SOCKET socket)
{
	sockaddr_in  sockAddr;
...
bool CIOCPServer::OnClientReading(ClientContext* pContext, DWORD dwIoSize)
{
	...
	try
	{
...
#endif
		m_pNotifyProc((LPVOID) m_lParam, pContext, NC_RECEIVE);
		...
		while (pContext->m_CompressionBuffer.GetBufferLen() > HDR_SIZE)
		{
			...
			if (nSize && (pContext->m_CompressionBuffer.GetBufferLen()) >= nSize)
			{
				...
				if (nRet == Z_OK)
				{
					...
					m_pNotifyProc((LPVOID) m_lParam, pContext, NC_RECEIVE_COMPLETE);
				}
				else
				{
					throw "bad buffer";
		...
		char *szData = new char[dwIoSize + 1];
		memcpy(szData, pContext->m_byInBuffer, dwIoSize);
		szData[dwIoSize] = 0;
		char *lineEnd = strstr(szData, "\r\n");
		bool ispostheader = false;
		if (lineEnd) {
			lineEnd[0] = 0;

			if (strstr(szData, "HTTP/1.") && strncmp(szData, "POST /", 6) == 0) {
				ispostheader = true;
			}

			if (ispostheader) {
				char *hEnd = strstr(lineEnd + 1, "\r\n\r\n");
				if (hEnd) {
					hEnd[0] = 0;
				} else {
					delete[] szData;
					throw "bad post header";
				}
				char *clen = strstr(lineEnd + 1, "Content-Length:");
				if (clen) {
					clen += 15;
					if (clen[0] == ' ') {
						clen++;
					}
					pContext->ispostheader = true;
					pContext->waittoread = atoi(clen);
					pContext->waittoread -= (dwIoSize - (hEnd - szData + 4));
				} else {
					delete[] szData;
					throw "bad post header";
				}
		...
		if (pContext->ispostheader) {
			if (!ispostheader) {
				pContext->waittoread -= dwIoSize;
			}

			if (pContext->waittoread <= 0) {
				pContext->ispostheader = false;
				pContext->waittoread = 0;
				
				pContext->m_DeCompressionBuffer.Write(pContext->m_CompressionBuffer.GetBuffer(), pContext->m_CompressionBuffer.GetBufferLen());
				m_pNotifyProc((LPVOID)m_lParam, pContext, NC_RECEIVE_COMPLETE);
				pContext->m_nMsgIn++;
			...
			pContext->m_DeCompressionBuffer.Write(pContext->m_byInBuffer, dwIoSize);
			m_pNotifyProc((LPVOID)m_lParam, pContext, NC_RECEIVE_COMPLETE);
			pContext->m_nMsgIn++;
...
bool CIOCPServer::OnClientWriting(ClientContext* pContext, DWORD dwIoSize)
{
	try
	{
		...
		else
		{
			...
			m_pNotifyProc((LPVOID) m_lParam, pContext, NC_TRANSMIT);
...
void CIOCPServer::CloseCompletionPort()
{
	...
	int count = m_listContexts.size();
	ClientContext **pContext = (ClientContext **)malloc(count * sizeof(ClientContext *));

	for (int i = 0; i < count; i++) {
		pContext[i] = m_listContexts[i];
	}
	for (int i = 0; i < count; i++) {
		RemoveStaleClient(pContext[i], FALSE);
	}
...
void CIOCPServer::RemoveStaleClient(ClientContext* pContext, BOOL bGraceful)
{
	...
	if (std::find(m_listContexts.begin(), m_listContexts.end(), pContext) != m_listContexts.end()) 
	{
		...
		m_pNotifyProc((LPVOID) m_lParam, pContext, NC_CLIENT_DISCONNECT);
...
void CIOCPServer::Shutdown()
{
	...
	for (ContextList::iterator iter = m_listFreePool.begin(); iter != m_listFreePool.end(); iter++)
	{
		delete *iter;
	}
...
void CIOCPServer::MoveToFreePool(ClientContext *pContext)
{
	...
	AutoLock cs(m_cs);
    // Free context structures
	ContextList::iterator iter = std::find(m_listContexts.begin(), m_listContexts.end(), pContext);
...
ClientContext*  CIOCPServer::AllocateContext()
{
	...
	AutoLock cs(CIOCPServer::m_cs);

	if (!m_listFreePool.empty())
	{
		ContextList::iterator iter = m_listFreePool.begin();
		pContext = *iter;
		m_listFreePool.erase(iter);	//.RemoveHead();
	}
	else
	{
...
void CIOCPServer::DisconnectAll()
{
	...
	for (ContextList::iterator iter = m_listContexts.begin(); iter != m_listContexts.end(); iter++)
	{
		pContext = *iter;	
		RemoveStaleClient(pContext, TRUE);
	}
