...
#if (_MSC_VER >= 1800)
    //VS2013
    VerifyVersionInfo(&version_info, VER_MAJORVERSION | VER_MINORVERSION | VER_PRODUCT_TYPE, NULL);
#else
...
void GetServicePackLevel(int* major, int* minor) {
  ...
  if (!checked_version) {
    OSVERSIONINFOEXW version_info = {0};
    version_info.dwOSVersionInfoSize = sizeof(version_info);
#if (_MSC_VER >= 1800)
    //VS2013
    VerifyVersionInfo(&version_info, VER_SERVICEPACKMAJOR | VER_SERVICEPACKMINOR, NULL);
#else
...
bool GetRealOSVersion(RTL_OSVERSIONINFOEXW *provi)
{
	return GetRealOSVersion((RTL_OSVERSIONINFOW *)provi);
}
